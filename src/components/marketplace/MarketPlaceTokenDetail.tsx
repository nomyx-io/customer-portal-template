...
export default function MarketPlaceTokenDetail({ token, tokens, currentIndex, next, prev, onBack, onPurchaseToken }: any) {
  ...
  useEffect(() => {
    ...
    if (currentTokenObj && currentTokenObj.token && currentTokenObj.token.projectId) {
      fetchProjectData(currentTokenObj.token.projectId);
    }
                  ...
                  <Card className="border dark:border-gray-700 border-gray-300 bg-gray-100 dark:bg-nomyx-dark2-dark p-6 rounded-lg shadow-md">
                    ...
                    <div className="mt-6">
                      ...
                      <button
                        ...
                        onClick={() => onPurchaseToken && onPurchaseToken(token)}
                      >
                        Purchase
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    {project && Array.isArray(project.attributes.fields) && project.attributes.fields.length > 0 ? (
                      project.attributes.fields.map((field: any) => {
                        console.log("Project Field:", project);
                        const value = token[field.key];
                        return (
                          <div key={field.key} className="flex items-center gap-4">
